...
public class SimulacaoDAO {
    ...
    public void cadastrarSimulacao(Simulacao simulacao) throws SQLException, ClassNotFoundException {
        String sql = "INSERT INTO t_gs_simulacao (usuario_id, tipo_cliente, localizacao, custo_mensal, orcamento, tipo_energia_recomendada, economia_anual, custo_instalacao_recomendada, tempo_retorno_recomendado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = ConnectionFactory.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setString(6, simulacao.getTipoEnergiaRecomendada());
            stmt.setFloat(7, simulacao.getEconomiaAnual());
            stmt.setFloat(8, simulacao.getCustoInstalacaoRecomendada());
            stmt.setFloat(9, simulacao.getTempoRetornoRecomendado());
            stmt.executeUpdate();
        }
    }
    ...
    public boolean atualizarSimulacao(Simulacao simulacao) throws SQLException, ClassNotFoundException {
        String sql = "UPDATE t_gs_simulacao SET usuario_id = ?, tipo_cliente = ?, localizacao = ?, custo_mensal = ?, orcamento = ?, tipo_energia_recomendada = ?, economia_anual = ?, custo_instalacao_recomendada = ?, tempo_retorno_recomendado = ? WHERE id = ?";
        try (Connection conn = ConnectionFactory.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, simulacao.getUsuarioId());
            stmt.setString(2, simulacao.getTipoCliente());
            ...
            stmt.setFloat(8, simulacao.getCustoInstalacaoRecomendada());
            stmt.setFloat(9, simulacao.getTempoRetornoRecomendado());
            stmt.setInt(10, simulacao.getId());
